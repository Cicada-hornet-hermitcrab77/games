...
def draw_costume(surface, char_name, head_c, hd, shoulder, waist, lh, rh, facing, s, col):
    ...
    if char_name == "Brawler":
        ...
        # Feathered bard hat
        pygame.draw.ellipse(surface, (150, 80, 180),
                            (hx - int(hd*0.9), hy - hd - int(hd*0.15), int(hd*1.8), int(hd*0.4)))
        pygame.draw.polygon(surface, (160, 90, 190),
                            [(hx - int(hd*0.7), hy - hd),
                             (hx + int(hd*0.7), hy - hd),
                             (hx + int(hd*0.5), hy - hd - int(hd*0.85)),
                             (hx - int(hd*0.5), hy - hd - int(hd*0.85))])
        # Feather on hat
        pygame.draw.line(surface, (255, 220, 100),
                         (hx + facing * int(hd*0.5), hy - hd - int(hd*0.1)),
                         (hx + facing * int(hd*1.1), hy - hd - int(hd*1.3)),
                         max(2, int(3*s)))
        # Musical note floating near hand
        _nx, _ny = int(rh[0]) + facing*int(14*s), int(rh[1]) - int(10*s)
        pygame.draw.circle(surface, (200, 120, 230), (_nx, _ny), max(3, int(4*s)))
        pygame.draw.line(surface, (200, 120, 230),
                         (_nx + max(2, int(3*s)), _ny),
                         (_nx + max(2, int(3*s)), _ny - int(10*s)),
                         max(1, int(2*s)))
        ...
        # Concrete-grey armored vest
        pygame.draw.rect(surface, (100, 95, 88),
                         (sx - int(9*s), sy, int(18*s), bl),
                         max(1, int(2*s)), border_radius=int(2*s))
        # Rivets
        for _ri in range(3):
            _ry = sy + int(bl * (0.2 + 0.3*_ri))
            pygame.draw.circle(surface, (140, 135, 128), (sx - int(7*s), _ry), max(2, int(2*s)))
            pygame.draw.circle(surface, (140, 135, 128), (sx + int(7*s), _ry), max(2, int(2*s)))
        # Stone crack lines on head
        pygame.draw.line(surface, (80, 75, 70),
                         (hx, hy - hd + int(2*s)), (hx + int(4*s), hy), max(1, int(s)))
        pygame.draw.line(surface, (80, 75, 70),
                         (hx - int(3*s), hy - int(hd*0.5)), (hx + int(2*s), hy + int(hd*0.3)),
                         max(1, int(s)))

    elif char_name == "Tycoon":
        # Top hat
        hat_w = int(hd * 1.4)
        pygame.draw.ellipse(surface, (180, 150, 0),
                            (hx - int(hd*0.95), hy - int(hd*0.3), int(hd*1.9), int(hd*0.4)))
        pygame.draw.rect(surface, (180, 150, 0),
                         (hx - hat_w//2, hy - hd - int(hd*0.9), hat_w, int(hd*0.95)))
        # Money bag symbol on chest
        _cx, _cy = sx, sy + int(bl * 0.38)
        pygame.draw.circle(surface, (220, 180, 0), (_cx, _cy), max(5, int(7*s)))
        pygame.draw.circle(surface, (160, 130, 0), (_cx, _cy), max(5, int(7*s)), max(1, int(s)))
        # $ sign
        pygame.draw.line(surface, (100, 80, 0),
                         (_cx, _cy - int(5*s)), (_cx, _cy + int(5*s)), max(1, int(s)))

    elif char_name == "Glass Jaw":
        # Cracked glass pattern on torso
        pygame.draw.rect(surface, (200, 190, 220),
                         (sx - int(8*s), sy, int(16*s), bl), max(1, int(s)), border_radius=int(2*s))
        for _qi, (_qx, _qy, _qex, _qey) in enumerate([
            (-6, 10, 4, 25), (3, 5, -5, 30), (-2, 20, 8, 38)]):
        ...
        # Playing card suit on chest
        _cx, _cy = sx, sy + int(bl * 0.38)
        pygame.draw.circle(surface, (80, 200, 80), (_cx, _cy), max(6, int(8*s)))
        # Diamond shape
        pygame.draw.polygon(surface, (220, 20, 20),
                            [(_cx, _cy - int(5*s)), (_cx + int(4*s), _cy),
                             (_cx, _cy + int(5*s)), (_cx - int(4*s), _cy)])
        # Dice on head
        pygame.draw.rect(surface, (240, 240, 240),
                         (hx - int(hd*0.5), hy - int(hd*0.45), int(hd), int(hd*0.9)),
                         border_radius=int(2*s))
        for _dox, _doy in ((-2, -2), (2, 2), (0, 0)):
            pygame.draw.circle(surface, (20, 20, 20),
                               (hx + int(_dox*s), hy + int(_doy*s)), max(1, int(2*s)))
        ...
        # Big chunky boots on feet (drawn below waist)
        for _bx, _by in [(lhx, lhy), (rhx, rhy)]:
            pygame.draw.rect(surface, (40, 50, 150),
                             (_bx - int(10*s), _by - int(6*s), int(20*s), int(14*s)),
                             border_radius=int(3*s))
            pygame.draw.rect(surface, (20, 30, 120),
                             (_bx - int(10*s), _by - int(6*s), int(20*s), int(14*s)),
                             max(1, int(2*s)), border_radius=int(3*s))
        # Impact star marks below boots
        for _ix, _iy, _ir in [(lhx - int(5*s), lhy + int(8*s), int(4*s)),
                               (rhx + int(5*s), rhy + int(8*s), int(4*s))]:
            for _ia in range(4):
                _iax = _ix + int(math.cos(_ia * 0.785) * _ir)
                _iay = _iy + int(math.sin(_ia * 0.785) * _ir)
                pygame.draw.line(surface, (80, 100, 220), (_ix, _iy), (_iax, _iay), max(1, int(s)))
        ...
        for _pi in range(6):
            ...
            _pdir = -1 if _pi % 2 == 0 else 1
            pygame.draw.polygon(surface, (50, 130, 40),
                                [(_px, _py - _pdir*int(10*s)),
                                 (_px - int(4*s), _py + _pdir*int(3*s)),
                                 (_px + int(4*s), _py + _pdir*int(3*s))])
        # Spikes on head
        for _hi2 in range(5):
            _ha = math.pi * (_hi2 / 4.0)
            _hspx = hx + int(math.cos(_ha) * hd)
            _hspy = hy - int(math.sin(_ha) * hd)
            pygame.draw.line(surface, (50, 130, 40),
                             (_hspx, _hspy),
                             (_hspx + int(math.cos(_ha)*8*s), _hspy - int(math.sin(_ha)*8*s)),
                             max(1, int(2*s)))

    elif char_name == "Anchor":
        # Anchor symbol on chest
        _cx, _cy = sx, sy + int(bl * 0.35)
        pygame.draw.circle(surface, (50, 50, 90), (_cx, _cy - int(6*s)), max(3, int(5*s)), max(1, int(2*s)))
        pygame.draw.line(surface, (50, 50, 90),
                         (_cx, _cy - int(1*s)), (_cx, _cy + int(10*s)), max(2, int(2*s)))
        pygame.draw.line(surface, (50, 50, 90),
                         (_cx - int(5*s), _cy + int(4*s)), (_cx + int(5*s), _cy + int(4*s)), max(2, int(2*s)))
        pygame.draw.line(surface, (50, 50, 90),
                         (_cx - int(5*s), _cy + int(10*s)), (_cx + int(5*s), _cy + int(10*s)), max(2, int(2*s)))
        # Heavy chain around waist
        for _chi in range(5):
            _chx = wx - int(10*s) + _chi * int(5*s)
            pygame.draw.ellipse(surface, (70, 70, 110),
                                (_chx, wy - int(3*s), int(6*s), int(5*s)), max(1, int(s)))
        ...
        # Faded mirror silhouette behind the character
        _toff = facing * int(18*s)
        _tsurf = pygame.Surface((hd*4, hd*8), pygame.SRCALPHA)
        pygame.draw.circle(_tsurf, (80, 180, 200, 60), (hd*2 + _toff, hd), hd)
        pygame.draw.line(_tsurf, (80, 180, 200, 60),
                         (hd*2 + _toff, hd*2), (hd*2 + _toff, hd*2 + int(bl)),
                         max(2, int(3*s)))
        surface.blit(_tsurf, (sx - hd*2 - _toff, sy - hd*2))
        # Cyan ring outline on head
        pygame.draw.circle(surface, (80, 180, 200), (hx, hy), hd, max(1, int(2*s)))

    elif char_name == "Sapper":
        # Dark circles under eyes
        for _eox in (-1, 1):
            pygame.draw.ellipse(surface, (60, 30, 10),
                                (hx + _eox*int(hd*0.25) - int(4*s), hy + int(hd*0.1),
                                 int(8*s), int(4*s)))
        # Drain arrows on arms (pointing inward toward body)
        for _ax, _ay, _adir in [(lhx, lhy, 1), (rhx, rhy, -1)]:
            _amx = (_ax + sx) // 2
            _amy = (_ay + sy) // 2
            pygame.draw.line(surface, (90, 45, 20),
                             (_ax, _ay), (_amx, _amy), max(1, int(2*s)))
            pygame.draw.polygon(surface, (90, 45, 20),
                                [(_amx + _adir*int(5*s), _amy),
                                 (_amx - _adir*int(3*s), _amy - int(4*s)),
                                 (_amx - _adir*int(3*s), _amy + int(4*s))])
        # Hollow gaunt look: dark outline on face
        pygame.draw.circle(surface, (70, 40, 15), (hx, hy), hd, max(1, int(2*s)))

    elif char_name == "Mimic":
        # Dashed outline around body
        for _di in range(8):
            _da = _di * 0.785
            _drx = hx + int(math.cos(_da) * hd * 1.35)
            _dry = hy + int(math.sin(_da) * hd * 1.35)
            pygame.draw.circle(surface, (150, 150, 160), (_drx, _dry), max(2, int(3*s)))
        # Question mark on chest
        _cx, _cy = sx, sy + int(bl * 0.3)
        pygame.draw.arc(surface, (150, 150, 160),
                        (_cx - int(6*s), _cy - int(8*s), int(12*s), int(10*s)),
                        math.pi * 0.1, math.pi * 1.1, max(2, int(2*s)))
        pygame.draw.line(surface, (150, 150, 160),
                         (_cx, _cy + int(2*s)), (_cx, _cy + int(5*s)), max(2, int(2*s)))
        pygame.draw.circle(surface, (150, 150, 160), (_cx, _cy + int(8*s)), max(1, int(2*s)))

    elif char_name == "Boomerang":
        # Curved boomerang weapon held in right hand
        pygame.draw.arc(surface, (200, 110, 30),
                        (rhx - int(14*s), rhy - int(10*s), int(28*s), int(20*s)),
                        0, math.pi, max(2, int(3*s)))
        pygame.draw.arc(surface, (230, 150, 60),
                        (rhx - int(10*s), rhy - int(7*s), int(20*s), int(14*s)),
                        0, math.pi, max(1, int(2*s)))
        # Orange arm-band on upper arm
        _bmx = (rhx + sx) // 2
        _bmy = (rhy + sy) // 2
        pygame.draw.circle(surface, (200, 110, 30), (_bmx, _bmy), max(3, int(5*s)))

    elif char_name == "Parry":
        # Blue buckler shield strapped to left arm
        ...
        # Silver visor line across head
        pygame.draw.line(surface, (180, 200, 240),
                         (hx - hd + 2, hy), (hx + hd - 2, hy), max(2, int(3*s)))
        ...
        pygame.draw.rect(surface, (100, 100, 120),
                         (sx - int(8*s), sy + int(4*s), int(16*s), int(bl // 2)))

    elif char_name == "Pierce":
        # Long silver lance tip extending from right hand
        _tip_x = rhx + facing * int(28*s)
        pygame.draw.line(surface, (180, 180, 200), (rhx, rhy), (_tip_x, rhy), max(2, int(3*s)))
        # Arrow head
        pygame.draw.polygon(surface, (220, 220, 240), [
            (_tip_x + facing * int(8*s), rhy),
            (_tip_x - facing * int(4*s), rhy - int(5*s)),
            (_tip_x - facing * int(4*s), rhy + int(5*s)),
        ])
        # Silver eye glint
        ...
        # Jagged red flames around fists when stacking
        for _fx, _fy in [(lhx, lhy), (rhx, rhy)]:
            for _fi in range(4):
                _fa = _fi * 1.57
                _fdx = int(math.cos(_fa) * 9 * s)
                _fdy = int(math.sin(_fa) * 9 * s)
                pygame.draw.line(surface, (200, 40, 20),
                                 (_fx, _fy), (_fx + _fdx, _fy + _fdy), max(1, int(2*s)))
        # Red headband
        band_y = hy + int(hd * 0.2)
        pygame.draw.line(surface, (200, 20, 20), (hx - hd, band_y), (hx + hd, band_y),
                         max(2, int(3*s)))

    elif char_name == "Phoenix":
        # Flame crest on top of head
        for _pi in range(3):
            _poff = (_pi - 1) * int(hd * 0.55)
            _ph = int((6 - abs(_pi - 1) * 2) * s) + hd
            pygame.draw.line(surface, (220, 120, 20),
                             (hx + _poff, hy - hd), (hx + _poff, hy - _ph),
                             max(1, int(2*s)))
            pygame.draw.circle(surface, (255, 180, 60), (hx + _poff, hy - _ph),
                                max(2, int(3*s)))
        # Wing hints behind torso
        pygame.draw.arc(surface, (220, 100, 20),
                        (sx - int(20*s) + int(10*s * facing), sy, int(20*s), int(bl // 2)),
                        math.pi * 0.1, math.pi * 0.9, max(2, int(3*s)))

    elif char_name == "Chain Fighter":
        # Chain links on both wrists
        ...
        # Chain across chest
        for _ci in range(4):
            _cpx = sx - int(6*s) + _ci * int(4*s)
            _cpy = sy + int(bl * 0.4)
            pygame.draw.circle(surface, (60, 160, 160), (_cpx, _cpy), max(1, int(3*s)))

    elif char_name == "Breaker":
        # Heavy purple gauntlets
        for _gx, _gy in [(lhx, lhy), (rhx, rhy)]:
            pygame.draw.rect(surface, (100, 80, 160),
                             (_gx - int(6*s), _gy - int(5*s), int(12*s), int(10*s)))
            pygame.draw.rect(surface, (140, 110, 200),
                             (_gx - int(6*s), _gy - int(5*s), int(12*s), int(10*s)), max(1, int(2*s)))
        # Cracked forehead line
        pygame.draw.line(surface, (100, 80, 160),
                         (hx - int(5*s), hy - int(hd * 0.5)),
                         (hx + int(3*s), hy + int(hd * 0.3)),
                         max(1, int(2*s)))

    elif char_name == "Titan Grip":
        # Large dark purple claw hands
        for _gx2, _gy2 in [(lhx, lhy), (rhx, rhy)]:
            pygame.draw.circle(surface, (90, 60, 130), (_gx2, _gy2), max(5, int(10*s)))
            # Claw lines
            for _ci2 in range(3):
                _ca = (_ci2 - 1) * 0.4
                pygame.draw.line(surface, (140, 100, 190),
                                 (_gx2, _gy2),
                                 (_gx2 + int(math.cos(_ca) * 10*s * (-facing)),
                                  _gy2 - int(abs(math.sin(_ca)) * 8*s)),
                                 max(1, int(2*s)))
        ...
        for _gli in range(6):
            _glx = hx + random.randint(-hd - 4, hd + 4)
            _gly = hy + random.randint(-hd - 4, hd + 4)
            _glc = [(60,255,60),(255,60,255),(60,200,255)][_gli % 3]
