...
def draw_costume(surface, char_name, head_c, hd, shoulder, waist, lh, rh, facing, s, col):
    ...
    if char_name == "Brawler":
    ...
    elif char_name in ("Eartha", "Spring Eartha", "Summer Eartha", "Autumn Eartha", "Winter Eartha"):
        _ev = char_name  # variant name

        # ── Shared base: rock head + crack lines ─────────────────────────────
        if _ev == "Winter Eartha":
            _head_stone = (175, 185, 205)   # frost-blue stone
            _crack_c    = (130, 145, 175)
        elif _ev == "Summer Eartha":
            _head_stone = (120, 95, 55)     # charred/dark bark (burning tree look)
            _crack_c    = (80, 55, 20)
        elif _ev == "Autumn Eartha":
            _head_stone = (140, 115, 90)    # earthy amber stone
            _crack_c    = (100, 75, 50)
        elif _ev == "Spring Eartha":
            _head_stone = (155, 148, 140)   # lighter spring stone
            _crack_c    = (100, 92, 82)
        else:
            _head_stone = (145, 135, 120)
            _crack_c    = (90, 85, 75)
        pygame.draw.circle(surface, _head_stone, (hx, hy), hd)
        pygame.draw.line(surface, _crack_c,
                         (hx - int(hd*0.3), hy - int(hd*0.6)),
        ...
        _by = sy
        _bw = int(18*s)
        if _ev == "Summer Eartha":
            _body_c  = (55, 35, 12)   # charred dark bark
            _grain_c = (90, 60, 22)
        elif _ev == "Autumn Eartha":
            _body_c  = (115, 65, 22)
            _grain_c = (145, 90, 40)
        elif _ev == "Winter Eartha":
            _body_c  = (80, 55, 28)   # bare pale wood
            _grain_c = (110, 78, 38)
        elif _ev == "Spring Eartha":
            _body_c  = (115, 72, 32)
            _grain_c = (145, 95, 50)
            ...
            _body_c  = (100, 60, 25)
            _grain_c = (130, 80, 35)
        pygame.draw.rect(surface, _body_c, (_bx, _by, _bw, bl), border_radius=max(1, int(2*s)))
        for _gi in range(3):
            _gx = _bx + int(_bw * (_gi * 0.28 + 0.15))
            pygame.draw.line(surface, _grain_c, (_gx, _by + int(bl*0.05)),
                             (_gx, _by + int(bl*0.9)), max(1, int(s)))

        # Summer: flame flickers on torso
        if _ev == "Summer Eartha":
            _ft2 = pygame.time.get_ticks()
            for _fi2 in range(4):
                _fa2  = math.radians(_ft2 * 0.22 + _fi2 * 90)
                _fr2  = int(hd * 0.45)
                _fax2 = sx + int(math.cos(_fa2) * _fr2 * 0.6)
                _fay2 = sy + int(math.sin(_fa2) * _fr2 * 0.4) + int(hd * 0.4)
                pygame.draw.circle(surface, (255, max(60, 140 - _fi2 * 20), 0),
                                   (_fax2, _fay2), max(3, int(hd * 0.20)))
        # Winter: snow patches on torso
        if _ev == "Winter Eartha":
            pygame.draw.ellipse(surface, (225, 235, 255),
                                (_bx, _by, _bw, int(5*s)))
            pygame.draw.ellipse(surface, (215, 228, 255),
                                (_bx + int(_bw*0.2), _by + int(bl*0.45), int(_bw*0.6), int(4*s)))
        ...
        if _ev == "Spring Eartha":
            _ecl, _ecr = (220, 120, 180), (200, 100, 220)
        elif _ev == "Summer Eartha":
            _ecl, _ecr = (255, 180, 0),   (255, 100, 0)
        elif _ev == "Autumn Eartha":
            _ecl, _ecr = (255, 140, 30),  (200, 75, 15)
        elif _ev == "Winter Eartha":
            _ecl, _ecr = (160, 215, 255), (120, 170, 235)
        else:
            _ecl, _ecr = (80, 220, 230),  (180, 80, 240)
        pygame.draw.polygon(surface, _ecl, [
            (_ex_l, _ey - _gr), (_ex_l - _gr, _ey),
            (_ex_l, _ey + _gr), (_ex_l + _gr, _ey)])
        pygame.draw.polygon(surface, _ecr, [
        ...
        if _ev == "Spring Eartha":
            ...
            for _li, _ang in enumerate((-0.6, 0.0, 0.6)):
                ...
                _lty = hy - int(hd * 1.9)
                pygame.draw.line(surface, (130, 75, 50), (_lbx, _lby), (_ltx, _lty), max(2, int(3*s)))
                for _pi in range(5):
                    _pa = math.radians(_pi * 72)
                    pygame.draw.circle(surface, (255, 175, 195),
                                       (_ltx + int(math.cos(_pa) * hd * 0.33),
                                        _lty + int(math.sin(_pa) * hd * 0.33)),
                                       max(2, int(hd * 0.17)))
                pygame.draw.circle(surface, (255, 230, 240), (_ltx, _lty), max(1, int(hd * 0.10)))
            # Bird nest sitting on top
            _nx, _ny = hx, hy - hd - int(hd * 0.15)
            _nw = int(hd * 1.0)
            pygame.draw.ellipse(surface, (110, 75, 35),
                                (_nx - _nw // 2, _ny - int(hd * 0.22), _nw, int(hd * 0.38)))
            pygame.draw.ellipse(surface, (140, 95, 45),
                                (_nx - _nw // 2, _ny - int(hd * 0.22), _nw, int(hd * 0.38)),
                                max(1, int(s * 1.5)))
            for _ei2 in range(3):
                _eox = int((_ei2 - 1) * hd * 0.3)
                pygame.draw.circle(surface, (200, 230, 200),
                                   (_nx + _eox, _ny - int(hd * 0.08)), max(2, int(hd * 0.18)))

        elif _ev == "Summer Eartha":
            # Burning tree crown — dark branches with fire tips
            for _li, _ang in enumerate((-0.7, -0.25, 0.25, 0.7)):
                _lbx = hx + int(math.sin(_ang) * hd * 0.4)
                _lby = hy - int(hd * 0.80)
                _ltx = hx + int(math.sin(_ang) * hd * 1.2)
                _lty = hy - int(hd * 1.85)
                pygame.draw.line(surface, (45, 28, 10), (_lbx, _lby), (_ltx, _lty), max(3, int(4*s)))
                # Fire at tip
                _ft3 = pygame.time.get_ticks()
                for _fk in range(3):
                    _fka = math.radians(_ft3 * 0.25 + _fk * 120 + _li * 30)
                    _fkr = max(2, int(hd * 0.22))
                    pygame.draw.circle(surface, (255, max(50, 160 - _fk * 40), 0),
                                       (_ltx + int(math.cos(_fka) * _fkr),
                                        _lty + int(math.sin(_fka) * _fkr)),
                                       max(2, int(hd * 0.16)))
            # Animated butterfly floating near the crown
            _bft  = pygame.time.get_ticks()
            _bfx  = hx - int(hd * 0.85)
            _bfy  = hy - int(hd * 2.1) + int(math.sin(_bft * 0.0025) * hd * 0.28)
            _bfw  = max(0.18, abs(math.sin(_bft * 0.005)))
            _bwr  = max(3, int(hd * 0.32))
            pygame.draw.ellipse(surface, (255, 145, 20),
                                (_bfx - max(2, int(_bwr * _bfw)), _bfy - _bwr // 2,
                                 max(2, int(_bwr * _bfw)), _bwr))
            pygame.draw.ellipse(surface, (255, 145, 20),
                                (_bfx, _bfy - _bwr // 2,
                                 max(2, int(_bwr * _bfw)), _bwr))
            pygame.draw.ellipse(surface, (200, 80, 10),
                                (_bfx - max(1, int(_bwr * _bfw * 0.6)), _bfy,
                                 max(1, int(_bwr * _bfw * 0.6)), max(2, _bwr // 2)))
            pygame.draw.ellipse(surface, (200, 80, 10),
                                (_bfx, _bfy,
                                 max(1, int(_bwr * _bfw * 0.6)), max(2, _bwr // 2)))
            pygame.draw.ellipse(surface, (30, 15, 5),
                                (_bfx - max(1, int(2*s)), _bfy - int(_bwr * 0.6),
                                 max(2, int(4*s)), int(_bwr * 1.2)))

        elif _ev == "Autumn Eartha":
            # Orange tree crown — big bushy canopy
            _acrown_r = int(hd * 1.55)
            _acrown_y = hy - hd - int(hd * 0.7)
            pygame.draw.circle(surface, (170, 75, 15), (hx, _acrown_y), _acrown_r)
            pygame.draw.circle(surface, (200, 100, 20), (hx - int(hd*0.5), _acrown_y - int(hd*0.3)),
                               int(_acrown_r * 0.72))
            pygame.draw.circle(surface, (215, 130, 25), (hx + int(hd*0.55), _acrown_y - int(hd*0.2)),
                               int(_acrown_r * 0.68))
            pygame.draw.circle(surface, (180, 90, 18), (hx, _acrown_y), _acrown_r, max(1, int(s)))
            # Bugs crawling on trunk (little dark dots with legs)
            for _bi2, (_bx2, _by2) in enumerate([(sx - int(5*s), sy + int(bl*0.25)),
                                                    (sx + int(6*s), sy + int(bl*0.55)),
                                                    (sx - int(3*s), sy + int(bl*0.75))]):
                pygame.draw.circle(surface, (20, 15, 5), (_bx2, _by2), max(2, int(2.5*s)))
                for _leg in (-1, 1):
                    pygame.draw.line(surface, (20, 15, 5),
                                     (_bx2, _by2),
                                     (_bx2 + int(_leg * 4*s), _by2 + int(2*s)), max(1, int(s)))
            # Animated falling leaves around head
            _at = pygame.time.get_ticks()
            for _li3 in range(5):
                _la  = (_at * 0.0008 + _li3 * 1.26) % (math.pi * 2)
                _lxo = int(math.sin(_la + _li3) * hd * 1.8)
                _lyo = int((_at * 0.05 + _li3 * 55) % (hd * 4)) - int(hd * 2)
                _lac = [(215, 75, 20), (195, 145, 25), (170, 55, 18)][_li3 % 3]
                pygame.draw.ellipse(surface, _lac,
                                    (hx + _lxo - int(4*s), hy + _lyo - int(5*s),
                                     int(8*s), int(6*s)))

        elif _ev == "Winter Eartha":
            # Snowy tree — bare branches laden with snow
            for _li, _ang in enumerate((-0.65, -0.2, 0.2, 0.65)):
                _lbx = hx + int(math.sin(_ang) * hd * 0.4)
                _lby = hy - int(hd * 0.80)
                _ltx = hx + int(math.sin(_ang) * hd * 1.1)
                _lty = hy - int(hd * 1.90)
                pygame.draw.line(surface, (58, 44, 30), (_lbx, _lby), (_ltx, _lty), max(3, int(3.5*s)))
                # Heavy snow on branch
                pygame.draw.ellipse(surface, (228, 238, 255),
                                    (_ltx - int(hd*0.32), _lty - int(hd*0.18),
                                     int(hd*0.64), int(hd*0.30)))
                # Icicle drip
                pygame.draw.line(surface, (175, 210, 255),
                                 (_ltx, _lty + int(hd * 0.12)),
                                 (_ltx, _lty + int(hd * 0.65)), max(2, int(2*s)))
                pygame.draw.circle(surface, (210, 232, 255),
                                   (_ltx, _lty + int(hd * 0.65)), max(2, int(hd * 0.11)))
            # Snow cap on head crown
            pygame.draw.ellipse(surface, (228, 238, 255),
                                (hx - hd, hy - hd - int(5*s), hd * 2, int(9*s)))
            # Robin sitting on the leftmost branch (red breast)
            _rb_ang = -0.65
            _rbx = hx + int(math.sin(_rb_ang) * hd * 1.1)
            _rby = hy - int(hd * 1.90) - int(hd * 0.32)
            _rb_r = max(4, int(hd * 0.30))
            pygame.draw.circle(surface, (65, 45, 25), (_rbx, _rby), _rb_r)            # dark body
            pygame.draw.circle(surface, (200, 80, 40),
                               (_rbx + int(facing * _rb_r * 0.35), _rby + int(_rb_r * 0.25)),
                               max(2, int(_rb_r * 0.55)))                               # red breast
            pygame.draw.circle(surface, (45, 35, 20), (_rbx, _rby - _rb_r + 1),
                               max(2, int(_rb_r * 0.45)))                               # head
            pygame.draw.line(surface, (50, 38, 20),
                             (_rbx + int(facing * _rb_r * 0.5), _rby + _rb_r - 1),
                             (_rbx + int(facing * _rb_r * 0.5), _rby + _rb_r + int(hd*0.18)),
                             max(1, int(s)))                                             # tiny leg
            # Animated snowflakes around body
            _wt = pygame.time.get_ticks()
            for _wi in range(6):
                _wa  = (_wt * 0.0006 + _wi * 1.05) % (math.pi * 2)
                _wxo = int(math.sin(_wa + _wi) * hd * 2.0)
                _wyo = int((_wt * 0.04 + _wi * 40) % (hd * 5)) - int(hd * 2.5)
                pygame.draw.circle(surface, (220, 235, 255),
                                   (hx + _wxo, hy + _wyo), max(1, int(2*s)))

        else:  # original Eartha
            _sq_branch_tip = None
            for _li, _ang in enumerate((-0.6, 0.0, 0.6)):
                _lbx = hx + int(math.sin(_ang) * hd * 0.5)
                _lby = hy - int(hd * 0.85)
                ...
                if _ang == 0.6:
                    _sq_branch_tip = (_ltx, _lty)
            # Squirrel on the rightmost branch
            if _sq_branch_tip:
                _sqx, _sqy = _sq_branch_tip
                _sqr = max(3, int(hd * 0.22))
                # Bushy tail (drawn first, behind body)
                pygame.draw.ellipse(surface, (175, 115, 50),
                                    (_sqx, _sqy - int(_sqr * 3.0),
                                     max(3, int(_sqr * 1.3)), max(4, int(_sqr * 2.6))))
                # Body
                pygame.draw.ellipse(surface, (130, 75, 30),
                                    (_sqx - _sqr, _sqy - int(_sqr * 0.7),
                                     max(3, int(_sqr * 2)), max(3, int(_sqr * 1.5))))
                # Head
                pygame.draw.circle(surface, (130, 75, 30),
                                   (_sqx, _sqy - int(_sqr * 1.4)), max(2, int(_sqr * 0.8)))
                # Ear nub
                pygame.draw.circle(surface, (130, 75, 30),
                                   (_sqx - int(_sqr * 0.35), _sqy - int(_sqr * 2.0)),
                                   max(1, int(_sqr * 0.32)))
                # Eye
    ...
    elif char_name == "Jawke":
        t   = pygame.time.get_ticks() / 1000.0
        _ap = t % 10.0   # animation phase in the 10-second cycle

        # ── Round colorful body ──────────────────────────────────────────────
        _br = int(bl * 0.52)
        _bcx, _bcy = sx, sy + bl // 2
        pygame.draw.circle(surface, (255, 180, 255), (_bcx, _bcy), _br)
        # Coloured wedge stripes
        for _ji in range(6):
            _ja = math.radians(_ji * 60)
            _jb = math.radians((_ji + 0.5) * 60)
            _jc = [(200, 60, 200), (255, 100, 100), (80, 80, 255),
                   (200, 60, 200), (255, 100, 100), (80, 80, 255)][_ji]
            pygame.draw.arc(surface, _jc,
                            (_bcx - _br, _bcy - _br, _br * 2, _br * 2),
                            _ja, _jb, max(4, int(6*s)))
        pygame.draw.circle(surface, (150, 40, 150), (_bcx, _bcy), _br, max(1, int(2*s)))

        # ── Small face on chest ──────────────────────────────────────────────
        _cfr = int(_br * 0.42)
        pygame.draw.circle(surface, (255, 220, 180), (_bcx, _bcy), _cfr)
        for _ced in (-1, 1):
            _cex = _bcx + _ced * int(_cfr * 0.38)
            _cey = _bcy - int(_cfr * 0.15)
            pygame.draw.circle(surface, (255, 255, 255), (_cex, _cey), max(2, int(4*s)))
            pygame.draw.circle(surface, (0, 0, 0),
                               (_cex + int(facing * int(s)), _cey + max(1, int(s))),
                               max(1, int(2*s)))
            pygame.draw.circle(surface, (180, 0, 0), (_cex, _cey), max(2, int(4*s)), max(1, int(s)))
        pygame.draw.arc(surface, (120, 0, 0),
                        (_bcx - int(_cfr * 0.45), _bcy + int(_cfr * 0.05),
        ...
        _hbase = hy - hd
        # Band
        pygame.draw.rect(surface, (160, 20, 160),
                         (hx - int(hd * 1.0), _hbase - int(4*s),
                          int(hd * 2.0), int(5*s)),
                         border_radius=max(1, int(2*s)))
        # Left tip (purple)
        _lt = (hx - int(hd * 0.3), _hbase - int(hd * 1.8))
        pygame.draw.polygon(surface, (180, 20, 180), [
            (hx - int(hd * 0.95), _hbase),
            (hx + int(hd * 0.1),  _hbase - int(4*s)),
            _lt,
        ])
        # Right tip (contrasting)
        _rt2 = (hx + int(hd * 0.3), _hbase - int(hd * 1.6)
                + int(math.sin(t * 3.5) * 3 * s))
        pygame.draw.polygon(surface, (80, 20, 200), [
            (hx - int(hd * 0.1), _hbase - int(4*s)),
            (hx + int(hd * 0.95), _hbase),
            _rt2,
        ])
        # Bells (gold, jiggling)
        for _tip, _phase in [(_lt, 0.0), (_rt2, 1.2)]:
            _boff = int(math.sin(t * 5 + _phase) * 4 * s)
            _bpos = (_tip[0], _tip[1] + _boff)
            pygame.draw.circle(surface, (255, 210, 0), _bpos, max(4, int(6*s)))
            pygame.draw.circle(surface, (180, 140, 0), _bpos, max(4, int(6*s)),
                               max(1, int(s)))
            pygame.draw.circle(surface, (40, 25, 0), _bpos, max(1, int(2*s)))

        # ── Head features: creepy clown ──────────────────────────────────────
        # Big white eyes with red outline
        for _ed in (-1, 1):
            _ex = hx + _ed * int(hd * 0.36)
            _ey = hy - int(hd * 0.12)
            pygame.draw.circle(surface, (255, 255, 255), (_ex, _ey), max(4, int(6*s)))
            pygame.draw.circle(surface, (0, 0, 0),
                               (_ex + int(facing * int(2*s)), _ey + max(1, int(s))),
                               max(2, int(3*s)))
            pygame.draw.circle(surface, (200, 0, 0), (_ex, _ey),
                               max(4, int(6*s)), max(1, int(s)))
        # Grin
        pygame.draw.arc(surface, (140, 0, 0),
                        (hx - int(hd * 0.62), hy + int(hd * 0.04),
                         int(hd * 1.24), int(hd * 0.66)),
                        math.pi, 2 * math.pi, max(2, int(3*s)))
        # Teeth
        for _ti in range(4):
            _tx = hx - int(hd * 0.48) + _ti * int(hd * 0.33)
            pygame.draw.line(surface, (230, 230, 230),
                             (_tx, hy + int(hd * 0.32)),
                             (_tx, hy + int(hd * 0.56)), max(1, int(2*s)))
        # Red nose
        pygame.draw.circle(surface, (220, 30, 30),
        ...
        if _in_tank:
            ...
            _foot_y = wy + int(LEG_LEN * s)
            _p_cx  = sx
            _p_top = hy - int(hd * 1.25)
            _b_top = _p_top
            _b_bot = wy + int(hd * 0.35)
            _b_cy  = (_b_top + _b_bot) // 2
            _b_rh  = (_b_bot - _b_top) // 2
            _b_rw  = int(_b_rh * 1.12)

            # ── 1. PUMPKIN HULL (upper body only — legs visible below) ────────
            # Side lobes
            for _lox in (-int(_b_rw * 0.70), int(_b_rw * 0.70)):
                _lrw = int(_b_rw * 0.54)
                _lrh = int(_b_rh * 0.87)
                pygame.draw.ellipse(surface, _pl,
                                    (_p_cx + _lox - _lrw, _b_cy - _lrh, _lrw * 2, _lrh * 2))
                pygame.draw.ellipse(surface, (190, 90, 12),
                                    (_p_cx + _lox - _lrw, _b_cy - _lrh, _lrw * 2, _lrh * 2),
                                    max(1, int(s)))
            # Inner secondary lobes
            for _lox in (-int(_b_rw * 0.43), int(_b_rw * 0.43)):
                _lrw = int(_b_rw * 0.46)
                _lrh = int(_b_rh * 0.95)
            ...
            pygame.draw.ellipse(surface, _pc,
                                (_p_cx - _b_rw, _b_cy - _b_rh, _b_rw * 2, _b_rh * 2))
            # Top highlight
            _hlsurf = pygame.Surface((_b_rw * 2, int(_b_rh * 1.1)), pygame.SRCALPHA)
            pygame.draw.ellipse(_hlsurf, (255, 170, 70, 70),
                                (int(_b_rw * 0.18), int(_b_rh * 0.06),
                                 int(_b_rw * 1.30), int(_b_rh * 0.58)))
            surface.blit(_hlsurf, (_p_cx - _b_rw, _b_cy - _b_rh))
            # Rib lines
            for _off in (-int(_b_rw * 0.55), -int(_b_rw * 0.22),
                          int(_b_rw * 0.22),  int(_b_rw * 0.55)):
                pygame.draw.line(surface, _pl,
                                 (_p_cx + _off, _b_cy - int(_b_rh * 0.90)),
                                 (_p_cx + _off, _b_cy + int(_b_rh * 0.90)),
                                 max(1, int(s)))
            # Hull outline
            pygame.draw.ellipse(surface, _pl,
                                (_p_cx - _b_rw, _b_cy - _b_rh, _b_rw * 2, _b_rh * 2),
                                max(2, int(2 * s)))
            # Cover stickman legs below hull
            ...
            for _sd in (-1, 1):
                _shx = _p_cx + _sd * _b_rw
                _shy = _b_cy - int(_b_rh * 0.14)
                if _sd == facing:
                    ...
                    pygame.draw.rect(surface, _mb,
                                     ...
                                     border_radius=max(2, int(s)))
                    pygame.draw.line(surface, _mh, (_bx0, _elby - int(_blw * 0.55)),
                                     (_bx1, _elby - int(_blw * 0.55)), max(1, int(s * 1.5)))
                    pygame.draw.line(surface, _mh, (_bx0, _elby + int(_blw * 0.55)),
                                     (_bx1, _elby + int(_blw * 0.55)), max(1, int(s * 1.5)))
            ...
            _fe_y = _b_cy - int(_b_rh * 0.18)
            for _fe_x in (_p_cx - int(_b_rw * 0.38), _p_cx + int(_b_rw * 0.38)):
            ...
            _m_y  = _b_cy + int(_b_rh * 0.24)
            _m_hw = int(_b_rw * 0.50)
            _seg6 = _m_hw * 2 // 6
            pygame.draw.lines(surface, (255, 165, 0), False, [
                (_p_cx - _m_hw,             _m_y),
            ...
            # ── 5. STEM + LEAF ────────────────────────────────────────────────
            _st_base = (_p_cx + int(hd * 0.1), _b_cy - _b_rh)
            _st_tip  = (_p_cx + int(hd * 0.35), _b_cy - _b_rh - int(hd * 1.0))
        ...
        _FEATHER_COLS = [
            ...
            (100, 40, 180),   # grape
            (255, 210, 30),   # banana
            (220, 30,  70),   # strawberry
            (255, 140, 20),   # orange
            (255, 100, 100),  # peach
            (255, 235, 50),   # lemon
            ( 80,  20, 100),  # blackberry
        ]
        _fan_cx = hx - facing * int(hd * 0.5)
        _fan_cy = wy - int(bl * 0.35)
        _fan_r  = int(hd * 2.8)
        _fan_start = math.pi * 0.38
        _fan_end   = math.pi * 1.62
        # Draw feathers back-to-front as proper blade shapes
        for _fi in range(10):
            ...
            _fdark = (max(0, _fc[0]-70), max(0, _fc[1]-70), max(0, _fc[2]-50))
            _fvx = _fx2 - _fan_cx;  _fvy = _fy2 - _fan_cy
            _flen = max(1, math.hypot(_fvx, _fvy))
            _fw = max(6, int(9*s))
            _px = int(-_fvy / _flen * _fw);  _py = int(_fvx / _flen * _fw)
            _q = 0.55   # blade starts 55% along shaft
            _qx = _fan_cx + int(_fvx * _q);  _qy = _fan_cy + int(_fvy * _q)
            _tip_x = _fx2 + int(_fvx / _flen * int(5*s))
            _tip_y = _fy2 + int(_fvy / _flen * int(5*s))
            # Shaft
            pygame.draw.line(surface, _fdark, (_fan_cx, _fan_cy), (_fx2, _fy2), max(2, int(2*s)))
            # Blade (teardrop polygon)
            _blade = [
                (_qx + _px, _qy + _py),
                (_tip_x, _tip_y),
                (_qx - _px, _qy - _py),
            ]
            pygame.draw.polygon(surface, _fc, _blade)
            # Blade highlight
            _light = (min(255, _fc[0]+70), min(255, _fc[1]+70), min(255, _fc[2]+60))
            _half_x = (_qx + _tip_x) // 2;  _half_y = (_qy + _tip_y) // 2
            pygame.draw.line(surface, _light, (_qx, _qy), (_half_x, _half_y), max(1, int(s)))
        # Body — plump round brown with belly highlight and wing detail
        pygame.draw.ellipse(surface, (105, 65, 22),
                            (sx - int(15*s), sy - int(2*s), int(30*s), bl + int(12*s)))
        pygame.draw.ellipse(surface, (145, 98, 42),
                            (sx - int(13*s), sy, int(26*s), bl + int(8*s)))
        # Belly highlight
        pygame.draw.ellipse(surface, (175, 130, 60),
                            (sx - int(7*s), sy + int(bl*0.28), int(14*s), int(bl*0.48)))
        # Wing curve (darker, back side)
        _wp = [
            (sx - facing*int(5*s),  sy + int(bl*0.12)),
            (sx - facing*int(15*s), sy + int(bl*0.38)),
            (sx - facing*int(14*s), sy + int(bl*0.72)),
            (sx - facing*int(6*s),  sy + int(bl*0.62)),
        ]
        pygame.draw.polygon(surface, (80, 48, 14), _wp)
        pygame.draw.polygon(surface, (120, 78, 32), _wp, max(1, int(s)))
        # Feather texture lines on wing
        for _wt in range(3):
            _wty = sy + int(bl * (0.2 + _wt * 0.18))
            pygame.draw.line(surface, (95, 60, 20),
                             (sx - facing*int(6*s), _wty),
                             (sx - facing*int(13*s), _wty + int(3*s)), max(1, int(s)))
        # Head — warm tan
        pygame.draw.circle(surface, (160, 100, 30), (hx, hy), hd + max(1, int(s)))
        pygame.draw.circle(surface, (195, 145, 62), (hx, hy), hd)
        # Snood (fleshy red droop above beak)
        _bk_x = hx + facing * int(hd * 0.82)
        pygame.draw.ellipse(surface, (190, 28, 28),
                            (_bk_x - int(5*s), hy - int(hd*0.52),
                             int(9*s), int(hd*0.62)))
        # Upper beak
        pygame.draw.polygon(surface, (215, 155, 30), [
            (_bk_x, hy - int(hd*0.1)),
            (_bk_x + facing*int(hd*0.58), hy + int(hd*0.02)),
            (_bk_x, hy + int(hd*0.16)),
        ])
        # Lower beak (slightly darker)
        pygame.draw.polygon(surface, (185, 130, 20), [
            (_bk_x, hy + int(hd*0.08)),
            (_bk_x + facing*int(hd*0.5), hy + int(hd*0.18)),
            (_bk_x, hy + int(hd*0.3)),
        ])
        # Wattle — elongated red blob hanging below beak
        pygame.draw.ellipse(surface, (185, 22, 22),
                            (_bk_x + facing*int(2*s) - int(4*s),
                             hy + int(hd*0.28), int(8*s), int(hd*0.5)))
        # Eye (large with iris and highlight)
        _ecx = hx + facing * int(hd*0.38)
        _ecy = hy - int(hd*0.18)
        pygame.draw.circle(surface, (20, 14, 6), (_ecx, _ecy), max(4, int(hd*0.3)))
        pygame.draw.circle(surface, (80, 50, 10), (_ecx, _ecy), max(3, int(hd*0.22)))
        pygame.draw.circle(surface, (255, 255, 255), (_ecx - int(s), _ecy - int(s)), max(1, int(hd*0.1)))

    elif char_name == "Nun-Gimel-Hei-Shin":
        # Spinning dreidel costume — large body dreidel + Star-of-David head
        _t    = pygame.time.get_ticks() / 750.0
        _spin = _t % (2 * math.pi)
        _dcx  = hx
        # Dreidel body spans from just below shoulder to above waist
        _d_top = sy + int(bl * 0.05)
        _d_bot = wy - int(bl * 0.05)
        _d_h   = _d_bot - _d_top
        _d_hw  = int(hd * 1.5)     # half-width of dreidel body
        # 3D shading: front face brighter than side
        _cos_a = math.cos(_spin)
        _face_b = int(120 + 80 * abs(_cos_a))   # brightness oscillates
        _c_front = (20, _face_b, min(255, _face_b + 60))
        _c_side  = (15, 55, 145)
        _c_edge  = (80, 180, 255)
        # Main dreidel body rectangle
        pygame.draw.rect(surface, _c_front,
                         (_dcx - _d_hw, _d_top, _d_hw * 2, _d_h),
                         border_radius=max(3, int(5*s)))
        # Left-side shading strip (depth illusion)
        _shade_w = max(4, int(_d_hw * 0.3))
        pygame.draw.rect(surface, _c_side,
                         (_dcx - _d_hw, _d_top, _shade_w, _d_h),
                         border_radius=max(3, int(5*s)))
        # Top highlight strip
        pygame.draw.rect(surface, _c_edge,
                         (_dcx - _d_hw, _d_top, _d_hw * 2, max(3, int(4*s))),
                         border_radius=max(3, int(5*s)))
        # Outline
        pygame.draw.rect(surface, _c_edge,
                         (_dcx - _d_hw, _d_top, _d_hw * 2, _d_h),
                         max(1, int(2*s)), border_radius=max(3, int(5*s)))
        # Point at bottom (spinning top tip)
        _tip_y = _d_bot + int(_d_hw * 0.95)
        pygame.draw.polygon(surface, _c_front, [
            (_dcx - _d_hw + int(4*s), _d_bot),
            (_dcx + _d_hw - int(4*s), _d_bot),
            (_dcx, _tip_y),
        ])
        pygame.draw.polygon(surface, _c_edge, [
            (_dcx - _d_hw + int(4*s), _d_bot),
            (_dcx + _d_hw - int(4*s), _d_bot),
            (_dcx, _tip_y),
        ], max(1, int(s)))
        # Hebrew letter Shin (ש) — three prongs with curved base
        _lc = (255, 215, 60)
        _lw = max(2, int(3*s))
        _lb_y  = _d_top + int(_d_h * 0.72)   # bottom of letter
        _lt_y  = _d_top + int(_d_h * 0.18)   # top of letter
        _lm_y  = _d_top + int(_d_h * 0.42)   # midpoint crossbar
        _lxl   = _dcx - int(_d_hw * 0.45)
        _lxm   = _dcx
        _lxr   = _dcx + int(_d_hw * 0.45)
        pygame.draw.line(surface, _lc, (_lxl, _lb_y), (_lxl, _lt_y), _lw)
        pygame.draw.line(surface, _lc, (_lxm, _lb_y), (_lxm, _lt_y + int(_d_h*0.12)), _lw)
        pygame.draw.line(surface, _lc, (_lxr, _lb_y), (_lxr, _lt_y), _lw)
        pygame.draw.line(surface, _lc, (_lxl, _lb_y), (_lxr, _lb_y), _lw)
        pygame.draw.line(surface, _lc, (_lxl, _lt_y), (_lxr, _lt_y), _lw)
        pygame.draw.line(surface, _lc, (_lxm, _lt_y + int(_d_h*0.12)), (_lxr, _lt_y), _lw)
        # Wooden handle (round-ended rod above body)
        _hnd_w = max(5, int(hd * 0.32))
        _hnd_top = hy - hd + int(hd * 0.1)
        _hnd_bot = _d_top
        pygame.draw.rect(surface, (155, 105, 42),
                         (_dcx - _hnd_w//2, _hnd_top, _hnd_w, _hnd_bot - _hnd_top))
        pygame.draw.circle(surface, (175, 125, 55), (_dcx, _hnd_top), _hnd_w // 2)
        pygame.draw.rect(surface, (110, 72, 24),
                         (_dcx - _hnd_w//2, _hnd_top, _hnd_w, _hnd_bot - _hnd_top),
                         max(1, int(s)))
        # Star of David on head (two overlapping triangles)
        _sd_r = max(5, int(hd * 0.6))
        _sd_c = (255, 215, 60)
        _sd_bg = (30, 110, 210)
        # Background fill (blue circle)
        pygame.draw.circle(surface, _sd_bg, (hx, hy), hd)
        _tri1 = [
            (hx, hy - _sd_r),
            (hx - int(_sd_r*0.866), hy + _sd_r//2),
            (hx + int(_sd_r*0.866), hy + _sd_r//2),
        ]
        _tri2 = [
            (hx, hy + _sd_r),
        ...
        pygame.draw.polygon(surface, _sd_bg, _tri1)
        pygame.draw.polygon(surface, _sd_bg, _tri2)
        pygame.draw.polygon(surface, _sd_c, _tri1, max(1, int(2*s)))
        ...
        for _bs in (-1, 1):
            _bx = sx + _bs * int(5*s)
            pygame.draw.rect(surface, (22, 18, 14),
                             (_bx - int(5*s), _boot_top, int(10*s), wy + int(LEG_LEN*s) - _boot_top),
                             border_radius=max(2, int(3*s)))
            pygame.draw.rect(surface, (50, 40, 30),
                             (_bx - int(5*s), _boot_top, int(10*s), max(3, int(4*s))))
        # Coat body — deep red with side shading
        pygame.draw.rect(surface, (155, 22, 22),
                         (sx - int(12*s), sy, int(24*s), bl + int(4*s)),
                         border_radius=max(2, int(4*s)))
        pygame.draw.rect(surface, (195, 35, 35),
                         (sx - int(10*s), sy, int(20*s), bl + int(2*s)),
                         border_radius=max(2, int(3*s)))
        # Coat center-line highlight (suggests lapels/opening)
        pygame.draw.line(surface, (140, 18, 18), (hx, sy + int(4*s)), (hx, wy), max(2, int(3*s)))
        # Buttons down center
        for _bi in range(3):
            _by = sy + int(bl * (0.2 + _bi * 0.25))
            pygame.draw.circle(surface, (255, 215, 0), (hx, _by), max(2, int(3*s)))
            pygame.draw.circle(surface, (200, 160, 0), (hx, _by), max(2, int(3*s)), max(1, int(s)))
        # White fur hem at bottom of coat
        pygame.draw.rect(surface, (215, 215, 215),
                         (sx - int(13*s), wy - int(2*s), int(26*s), int(8*s)),
                         border_radius=max(1, int(3*s)))
        # Fluffy texture on hem (wavy dots)
        for _hi in range(6):
            _hbx = sx - int(10*s) + _hi * int(4*s)
            pygame.draw.circle(surface, (240, 240, 240), (_hbx, wy + int(2*s)), max(2, int(3*s)))
        # White fur collar
        pygame.draw.rect(surface, (215, 215, 215),
                         (sx - int(7*s), sy, int(14*s), int(9*s)),
                         border_radius=max(1, int(3*s)))
        # White fur cuffs at wrists (near hand positions)
        for _hpos in ((lhx, lhy), (rhx, rhy)):
            pygame.draw.circle(surface, (215, 215, 215), _hpos, max(4, int(6*s)))
            pygame.draw.circle(surface, (240, 240, 240), _hpos, max(4, int(6*s)), max(1, int(s)))
        # Wide black belt
        _belt_y = sy + int(bl * 0.56)
        pygame.draw.rect(surface, (25, 18, 12),
                         (sx - int(11*s), _belt_y, int(22*s), int(9*s)))
        # Gold buckle (rectangle with inner rectangle)
        pygame.draw.rect(surface, (200, 165, 0),
                         (hx - int(7*s), _belt_y + int(1*s), int(14*s), int(7*s)),
                         border_radius=max(1, int(2*s)))
        pygame.draw.rect(surface, (255, 215, 30),
                         (hx - int(4*s), _belt_y + int(3*s), int(8*s), int(3*s)))
        # Santa hat — cone with brim and pompom
        _hat_brim_y = hy - int(hd * 0.75)
        _hat_bl = hx - hd - int(6*s)
        _hat_br = hx + hd + int(6*s)
        _hat_tip = (hx + facing * int(hd * 0.6), hy - int(hd * 2.5))
        pygame.draw.polygon(surface, (155, 22, 22), [
            (_hat_bl, _hat_brim_y),
            (_hat_br, _hat_brim_y),
            _hat_tip,
        ])
        pygame.draw.polygon(surface, (195, 35, 35), [
            (_hat_bl + int(4*s), _hat_brim_y),
            (_hat_br - int(4*s), _hat_brim_y),
            (hx + facing*int(hd*0.4), hy - int(hd*2.0)),
        ])
        # Hat brim band
        pygame.draw.rect(surface, (215, 215, 215),
                         (_hat_bl, _hat_brim_y, _hat_br - _hat_bl, int(hd*0.45)),
                         border_radius=max(1, int(3*s)))
        pygame.draw.rect(surface, (240, 240, 240),
                         (_hat_bl, _hat_brim_y, _hat_br - _hat_bl, max(3, int(4*s))),
                         border_radius=max(1, int(3*s)))
        # Pompom (three overlapping circles)
        for _pp in ((-int(2*s), -int(2*s)), (int(2*s), 0), (0, int(2*s))):
            pygame.draw.circle(surface, (235, 235, 235),
                               (_hat_tip[0] + _pp[0], _hat_tip[1] + _pp[1]),
                               max(4, int(hd*0.38)))
        pygame.draw.circle(surface, (255, 255, 255), _hat_tip, max(3, int(hd*0.28)))
        # Rosy cheeks
        for _sign in (-1, 1):
            pygame.draw.circle(surface, (215, 90, 72),
                               (hx + _sign * int(hd*0.5), hy + int(hd*0.12)),
                               max(3, int(hd*0.28)))
        # White beard — layered curls
        _beard_base = [
            (hx - int(hd*0.7), hy + int(hd*0.08)),
            (hx + int(hd*0.7), hy + int(hd*0.08)),
            (hx + int(hd*0.65), hy + int(hd*1.0)),
            (hx + int(hd*0.2),  hy + int(hd*1.25)),
            (hx,                hy + int(hd*1.35)),
            (hx - int(hd*0.2),  hy + int(hd*1.25)),
            (hx - int(hd*0.65), hy + int(hd*1.0)),
        ]
        pygame.draw.polygon(surface, (210, 210, 210), _beard_base)
        pygame.draw.polygon(surface, (240, 240, 240), _beard_base, max(1, int(s)))
        # Curl lines on beard
        for _ci in range(4):
            _cx = hx - int(hd*0.5) + _ci * int(hd*0.33)
            pygame.draw.arc(surface, (180, 180, 180),
                            pygame.Rect(_cx - int(4*s), hy + int(hd*(0.35 + _ci*0.18)),
                                        int(8*s), int(6*s)),
                            0, math.pi, max(1, int(s)))
        # Eyes — small and jolly
        for _ex in (hx - int(hd*0.3), hx + int(hd*0.3)):
            pygame.draw.circle(surface, (28, 22, 14), (_ex, hy - int(hd*0.22)), max(2, int(hd*0.18)))
            pygame.draw.circle(surface, (255, 255, 255), (_ex + int(s), hy - int(hd*0.28)),
                               max(1, int(hd*0.07)))

    elif char_name == "Baddit":
        # Tall bunny ears (drawn behind head)
        for _esign in (-1, 1):
            _ear_cx  = hx + _esign * int(hd * 0.42)
            _ear_bot = hy - int(hd * 0.72)
            _ear_top = hy - int(hd * 3.0)
            _ear_h   = _ear_bot - _ear_top
            _ear_w   = max(6, int(hd * 0.4))
            # Outer ear (creamy white)
            pygame.draw.ellipse(surface, (225, 208, 188),
                                (_ear_cx - _ear_w, _ear_top, _ear_w * 2, _ear_h))
            pygame.draw.ellipse(surface, (200, 182, 162),
                                (_ear_cx - _ear_w, _ear_top, _ear_w * 2, _ear_h),
                                max(1, int(s)))
            # Inner ear (warm pink)
            _iw = max(3, int(_ear_w * 0.52))
            _ih = int(_ear_h * 0.68)
            pygame.draw.ellipse(surface, (245, 150, 170),
                                (_ear_cx - _iw, _ear_top + int(_ear_h * 0.12), _iw * 2, _ih))
            # Ear highlight
            pygame.draw.ellipse(surface, (255, 185, 200),
                                (_ear_cx - max(1, int(_iw*0.4)), _ear_top + int(_ear_h*0.14),
                                 max(2, int(_iw*0.8)), int(_ih*0.4)))
        # Bandit eye mask (wide, rounded, slightly angled)
        _mask_y = hy - int(hd * 0.38)
        _mask_h = int(hd * 0.55)
        pygame.draw.rect(surface, (28, 18, 18),
                         (hx - hd - int(4*s), _mask_y, (hd + int(4*s)) * 2, _mask_h),
                         border_radius=max(3, int(4*s)))
        # Mask tie string on side (little knot)
        _knot_x = hx + facing * (hd + int(5*s))
        pygame.draw.circle(surface, (40, 28, 28), (_knot_x, _mask_y + _mask_h//2),
                           max(2, int(3*s)))
        pygame.draw.line(surface, (40, 28, 28), (_knot_x, _mask_y + _mask_h//2),
                         (hx + facing*(hd + int(9*s)), _mask_y + _mask_h//2),
                         max(1, int(s)))
        # Eye holes — round cutouts in the mask
        for _exi, _ex in enumerate((hx - int(hd*0.38), hx + int(hd*0.38))):
            _ey2 = _mask_y + _mask_h // 2
            pygame.draw.circle(surface, (240, 220, 200), (_ex, _ey2), max(4, int(hd*0.26)))
            # Iris
            pygame.draw.circle(surface, (55, 38, 20), (_ex, _ey2), max(3, int(hd*0.18)))
            # Pupil
            pygame.draw.circle(surface, (12, 8, 6), (_ex, _ey2), max(2, int(hd*0.1)))
            # Eye highlight
            pygame.draw.circle(surface, (255, 255, 255),
                               (_ex + int(s), _ey2 - int(s)), max(1, int(hd*0.07)))
        # Bunny nose (heart-ish: two circles + triangle)
        _nx = hx;  _ny = hy + int(hd * 0.28)
        _nr = max(2, int(hd * 0.14))
        pygame.draw.circle(surface, (235, 110, 140), (_nx - _nr, _ny), _nr)
        pygame.draw.circle(surface, (235, 110, 140), (_nx + _nr, _ny), _nr)
        pygame.draw.polygon(surface, (235, 110, 140), [
            (_nx - _nr, _ny), (_nx + _nr, _ny), (_nx, _ny + _nr + int(s))
        ])
        # Whiskers (3 per side, spreading out)
        for _wdir in (-1, 1):
            for _wi in range(3):
                _wang = math.radians(_wi * 18 - 18)
                _wlen = int(hd * 1.0)
                _wy3  = hy + int(hd * (0.15 + _wi * 0.1))
                _wx2  = hx + _wdir * int(math.cos(_wang) * _wlen)
                _wy2  = _wy3 + int(math.sin(_wang) * _wlen * _wdir)
                pygame.draw.line(surface, (120, 95, 75),
                                 (hx + _wdir * int(hd*0.1), _wy3),
                                 (_wx2, _wy2), max(1, int(s)))
        # Small thief sack over shoulder (circle with tie)
        _sack_x = sx - facing * int(8*s)
        _sack_y = sy + int(bl * 0.15)
        _sack_r = max(5, int(hd * 0.55))
        pygame.draw.circle(surface, (160, 130, 80), (_sack_x, _sack_y), _sack_r)
        pygame.draw.circle(surface, (130, 102, 55), (_sack_x, _sack_y), _sack_r, max(1, int(s)))
        # Sack tie/knot at top
        pygame.draw.circle(surface, (100, 78, 40),
                           (_sack_x, _sack_y - _sack_r + int(3*s)), max(2, int(3*s)))
        # Dollar sign on sack
        pygame.draw.line(surface, (210, 175, 80),
                         (_sack_x, _sack_y - int(4*s)), (_sack_x, _sack_y + int(4*s)), max(1, int(s)))
        pygame.draw.arc(surface, (210, 175, 80),
                        pygame.Rect(_sack_x - int(3*s), _sack_y - int(4*s), int(6*s), int(5*s)),
                        math.pi * 0.1, math.pi * 1.1, max(1, int(s)))
        pygame.draw.arc(surface, (210, 175, 80),
...
def draw_stickman(surface, x, y, color, facing, action, action_t, flash=False, scale=1.0, char_name=""):
    ...
    if char_name in ("Booma", "Testa di Testy", "Boxy", "Supa"):
        ...
        _bob = int(math.sin(_dt2 * 3.0 + x * 0.02) * 2 * s) if action in ('idle', 'walk') else 0

        if action == 'dead':
            _pcy = int(y) - int(_pr * 0.55)
            pygame.draw.ellipse(surface, (60, 48, 30), (int(x) - int(_pr*1.3), int(y) - int(hd*0.3), int(_pr*2.6), int(hd*0.6)))
            pygame.draw.circle(surface, (150, 120, 70), (int(x) - facing*int(_pr*0.4), _pcy), int(_pr*0.9))
            pygame.draw.circle(surface, (60, 20, 15), (int(x) - facing*int(_pr*0.6), _pcy - int(_pr*0.1)), max(1, int(_pr*0.15)))
            return None

        _pcy = int(y) - int(_pr * 1.15 * _duck_sq) - _bob
        _potato_pts = [
        ...
        _pts = [(int(x) + int(px * _pr), _pcy + int(py * _pr * _duck_sq)) for px, py in _potato_pts]
        ...
        for _sxo, _syo in [(-0.4, -0.5), (0.5, -0.2), (-0.1, 0.4), (0.4, 0.55)]:
            pygame.draw.circle(surface, _edge_col, (int(x) + int(_sxo*_pr), _pcy + int(_syo*_pr*_duck_sq)), max(1, int(hd*0.08)))
        # Rodent-y face: beady eyes + buck teeth
        _fey = _pcy - int(_pr * 0.5 * _duck_sq)
        _feo = max(2, int(hd * 0.22))
        for _fexo in (-1, 1):
            pygame.draw.circle(surface, (30, 20, 12), (int(x) + _fexo * int(_pr*0.28), _fey), _feo)
        _fty = _pcy - int(_pr * 0.05)
        ...
        if char_name == "Booma":
            ...
            for _dxo in (-1, 1):
                _dx = int(x) + _dxo * int(_pr * 0.75)
                pygame.draw.rect(surface, (180, 30, 30), (_dx - int(3*s), _pcy - int(_pr*0.3), int(6*s), int(_pr*0.5)))
                pygame.draw.line(surface, (150, 110, 60), (_dx, _pcy - int(_pr*0.3)),
                                 (_dx, _pcy - int(_pr*0.55)), max(1, int(2*s)))
        elif char_name == "Testa di Testy":
            _gy = _pcy - int(_pr * 0.55)
            _glass = (140, 220, 230)
            for _gxo in (-1, 1):
                _gx = int(x) + _gxo * int(_pr * 0.32)
                pygame.draw.circle(surface, (60, 60, 65), (_gx, _gy), max(2, int(hd*0.4)))
                pygame.draw.circle(surface, _glass, (_gx, _gy), max(1, int(hd*0.3)))
            pygame.draw.line(surface, (60, 60, 65), (int(x) - int(_pr*0.1), _gy), (int(x) + int(_pr*0.1), _gy), max(2, int(3*s)))
            _bkx, _bky = int(x) + int(_pr * 0.9), _pcy + int(_pr * 0.3)
            ...
            _cape_pts = [
                (int(x) - _cdir*int(_pr*0.5), _pcy - int(_pr*0.4)),
                (int(x) + _cdir*int(_pr*0.5), _pcy - int(_pr*0.4)),
                (int(x) + _cdir*int(_pr*1.3), int(y) + int(hd*0.3)),
                (int(x) - _cdir*int(_pr*0.2), int(y) + int(hd*0.7)),
            ]
    ...
    if char_name == "Splaut & Dusty":
        ...
        _cx9, _cy9 = int(x), int(y) - int(hd * 1.6) - _bob
        _br = int(hd * 1.75)
        _dk = tuple(max(0,c-35) for c in col)
        _lt = tuple(min(255,c+35) for c in col)

        # Ground contact shadow
        _shsurf = pygame.Surface((_br*2+8, int(_br*0.7)), pygame.SRCALPHA)
        pygame.draw.ellipse(_shsurf, (0,0,0,70), (0,0,_br*2+8,int(_br*0.7)))
        surface.blit(_shsurf, (_cx9-_br-4, int(y)-int(_br*0.35)))

        # Webbed feet — drawn first so the body overlaps them
        for _fxo in (-int(_br*0.55), int(_br*0.55)):
            _fx9, _fy9 = _cx9+_fxo, int(y)
            _fweb = pygame.Surface((int(30*s), int(16*s)), pygame.SRCALPHA)
            pygame.draw.polygon(_fweb, _dk, [(0,int(16*s)),(int(15*s),0),(int(30*s),int(16*s))])
            for _toe in range(4):
                _tx0 = int(_toe*10*s)
                pygame.draw.line(_fweb, col, (int(15*s),0), (_tx0,int(16*s)), max(1,int(2*s)))
            surface.blit(_fweb, (_fx9-int(15*s), _fy9-int(4*s)))

        # Body — gradient-ish shaded sphere with mottled spots
        pygame.draw.circle(surface, _dk, (_cx9, _cy9), _br)
        pygame.draw.circle(surface, col, (_cx9, _cy9-int(_br*0.08)), int(_br*0.92))
        pygame.draw.circle(surface, _lt, (_cx9-int(_br*0.32), _cy9-int(_br*0.38)), int(_br*0.34))
        for _sxo,_syo,_sr in [(-0.45,0.15,0.16),(0.35,0.35,0.13),(0.05,0.55,0.11),(0.5,-0.05,0.1)]:
            pygame.draw.circle(surface, _dk, (_cx9+int(_sxo*_br), _cy9+int(_syo*_br)), max(2,int(_sr*_br)))
        pygame.draw.circle(surface, _dk, (_cx9, _cy9), _br, max(1,int(2*s)))

        # Big expressive eyes that track the opponent's general direction
        for _exo in (-0.36, 0.36):
            _ex = _cx9+int(_exo*_br); _ey = _cy9-int(_br*0.32)
            pygame.draw.circle(surface, (255,255,255), (_ex,_ey), max(4,int(hd*0.34)))
            pygame.draw.circle(surface, (25,20,15), (_ex+facing*int(2*s),_ey), max(2,int(hd*0.16)))
            pygame.draw.circle(surface, (255,255,255), (_ex+facing*int(3*s)-1,_ey-int(2*s)), max(1,int(hd*0.05)))
        # Nostril slits
        for _nxo in (-0.1,0.1):
            pygame.draw.line(surface, _dk, (_cx9+int(_nxo*_br), _cy9+int(_br*0.05)),
                             (_cx9+int(_nxo*_br), _cy9+int(_br*0.15)), max(1,int(2*s)))

        # 3 forked tongues around the mouth, flick out and spit sand
        _spit_cycle = _pt % 1.8
        _spitting = _spit_cycle < 0.3
        _flick = math.sin((_spit_cycle/0.3)*math.pi) if _spitting else 0.0
        for _ti, _tang in enumerate((-0.55, 0.0, 0.55)):
            _tbx = _cx9 + facing*int(_br*0.75*math.cos(_tang))
            _tby = _cy9 + int(_br*0.4) + int(_br*0.32*math.sin(_tang))
            _tlen = int((9 + _flick*24) * s)
            _midx = _tbx + facing*int(_tlen*0.7)
            _ttx = _tbx + facing*_tlen
            _twav = int(math.sin(_pt*8+_ti)*3*s)
            pygame.draw.line(surface, (235,70,95), (_tbx,_tby), (_midx,_tby+_twav), max(2,int(4*s)))
    ...
    if char_name == "Bloob & Beatrix":
        ...
        _base_y = int(y)
        _cycle_t = _bt % 5.0
        _morph = min(1.0, _cycle_t / 0.4) if _cycle_t < 0.4 else (1.0 - min(1.0,(_cycle_t-4.6)/0.4) if _cycle_t > 4.6 else 1.0)
        _form = int(_bt // 5) % 3   # 0=ball 1=stickman 2=motorcycle
        _blob_col = col
        _blob_dk  = tuple(max(0,c-30) for c in col)
        _blob_lt  = tuple(min(255,c+40) for c in col)

        # Ground shadow
        _shsurf = pygame.Surface((int(hd*3.6), int(hd*1)), pygame.SRCALPHA)
        pygame.draw.ellipse(_shsurf, (0,0,0,70), (0,0,int(hd*3.6),int(hd*1)))
        surface.blit(_shsurf, (_cx9-int(hd*1.8), _base_y-int(hd*0.5)))

        if _form == 0:
            # Ball form — glossy slime sphere with wobbling tentacles
            _r = int(hd * 1.7 * (1.0 - _morph*0.15))
            _cy9 = _base_y - _r
            pygame.draw.circle(surface, _blob_dk, (_cx9, _cy9), _r)
            pygame.draw.circle(surface, _blob_col, (_cx9, _cy9-int(_r*0.1)), int(_r*0.9))
            pygame.draw.circle(surface, _blob_lt, (_cx9-int(_r*0.3), _cy9-int(_r*0.35)), int(_r*0.3))
            for _tsign in (-1,1):
                _tang = math.sin(_bt*3+_tsign)*0.4
                _ttx = _cx9+_tsign*int(_r*0.75*math.cos(_tang)); _tty = _cy9+int(_r*0.9)+int(_r*0.3*math.sin(_tang))
                pygame.draw.line(surface, _blob_dk, (_cx9+_tsign*int(_r*0.55), _cy9+int(_r*0.55)), (_ttx,_tty), max(3,int(6*s)))
                pygame.draw.circle(surface, _blob_dk, (_ttx,_tty), max(2,int(4*s)))
            _rider_y = _cy9 - _r
        elif _form == 1:
            # Stickman form — a simplified slime humanoid, drippy at the joints
            _bl9 = int(hd*2.5)
            _cy9 = _base_y - _bl9
            pygame.draw.line(surface, _blob_dk, (_cx9,_base_y), (_cx9,_cy9), max(6,int(11*s)))
            pygame.draw.line(surface, _blob_col, (_cx9-int(2*s),_base_y), (_cx9-int(2*s),_cy9), max(3,int(7*s)))
            pygame.draw.circle(surface, _blob_col, (_cx9,_cy9), int(hd*0.95))
            pygame.draw.circle(surface, _blob_lt, (_cx9-int(hd*0.25),_cy9-int(hd*0.25)), int(hd*0.3))
            for _lsign in (-1,1):
                _shy = _base_y-_bl9//3
                pygame.draw.line(surface, _blob_dk, (_cx9, _shy), (_cx9+_lsign*int(hd*1.15), _base_y), max(3,int(6*s)))
                pygame.draw.line(surface, _blob_dk, (_cx9, _cy9+int(hd*0.65)), (_cx9+_lsign*int(hd*1.05), _cy9+int(hd*1.7)), max(3,int(6*s)))
                pygame.draw.circle(surface, _blob_col, (_cx9+_lsign*int(hd*1.05), _cy9+int(hd*1.7)), max(2,int(3*s)))
            _rider_y = _cy9 - int(hd*0.95)
        else:
            # Motorcycle form — chrome wheels, slime-green chassis, headlight
            _mw, _mh = int(hd*3.4), int(hd*1.15)
            _my9 = _base_y - int(hd*0.75)
            for _wxo in (-facing*_mw//2, facing*_mw//2):
                pygame.draw.circle(surface, (25,25,28), (_cx9+_wxo, _base_y), int(hd*0.85))
                pygame.draw.circle(surface, (90,90,98), (_cx9+_wxo, _base_y), int(hd*0.85), max(1,int(3*s)))
                pygame.draw.circle(surface, (60,60,66), (_cx9+_wxo, _base_y), int(hd*0.3))
            pygame.draw.rect(surface, _blob_dk, (_cx9-_mw//2, _my9-_mh//2, _mw, _mh), border_radius=max(2,int(7*s)))
            pygame.draw.rect(surface, _blob_col, (_cx9-_mw//2, _my9-_mh//2, _mw, int(_mh*0.5)), border_radius=max(2,int(7*s)))
            pygame.draw.rect(surface, _blob_dk, (_cx9-_mw//2, _my9-_mh//2, _mw, _mh), max(1,int(2*s)), border_radius=max(2,int(6*s)))
            _hlx = _cx9+facing*int(_mw*0.42)
            pygame.draw.circle(surface, (255,240,180), (_hlx, _my9), max(3,int(hd*0.28)))
            pygame.draw.circle(surface, (200,180,100), (_hlx, _my9), max(3,int(hd*0.28)), max(1,int(2*s)))
            pygame.draw.line(surface, (40,38,42), (_cx9-facing*int(_mw*0.3), _my9-_mh//2),
                             (_cx9-facing*int(_mw*0.5), _my9-int(_mh*1.3)), max(2,int(5*s)))
            _rider_y = _my9 - int(_mh*1.4)
        ...
        # Antennae
        for _asign in (-1,1):
            pygame.draw.line(surface, (30,20,20), (_rx9+_asign*int(_bug_r*0.3), _rider_y-_bug_r),
                             (_rx9+_asign*int(_bug_r*0.9), _rider_y-int(_bug_r*1.7)), max(1,int(2*s)))
            pygame.draw.circle(surface, (220,60,60), (_rx9+_asign*int(_bug_r*0.9), _rider_y-int(_bug_r*1.7)), max(1,int(2*s)))
        # Body (shell) + helmet
        pygame.draw.circle(surface, (170,35,35), (_rx9, _rider_y), _bug_r)
        pygame.draw.circle(surface, (120,20,20), (_rx9, _rider_y), _bug_r, max(1,int(2*s)))
        pygame.draw.line(surface, (120,20,20), (_rx9, _rider_y-_bug_r), (_rx9, _rider_y+_bug_r), max(1,int(2*s)))
        pygame.draw.circle(surface, (215,215,220), (_rx9, _rider_y-int(_bug_r*0.2)), int(_bug_r*0.75), max(1,int(2*s)))
        pygame.draw.arc(surface, (255,255,255), (_rx9-int(_bug_r*0.7), _rider_y-int(_bug_r*0.9), int(_bug_r*1.2), int(_bug_r*0.9)),
                        math.radians(120), math.radians(200), max(1,int(2*s)))
        # Little bug legs
        for _lxo in (-1,1):
            pygame.draw.line(surface, (40,30,30), (_rx9,_rider_y+int(_bug_r*0.3)),
                             (_rx9+_lxo*int(_bug_r*1.1), _rider_y+int(_bug_r*0.9)), max(1,int(2*s)))
    ...
    # ── Rook & Moosh: a pirate rock on a wooden peg leg, mushroom sidekick ───
    #    riding on top. Rook wobbles as he tries to balance every 4 seconds.
    if char_name == "Rook & Moosh":
        _rt = pygame.time.get_ticks() / 1000.0
        if action == 'dead':
            return None
        _rock_col = col
        _rock_dk  = tuple(max(0,c-35) for c in col)
        _rock_lt  = tuple(min(255,c+35) for c in col)
        _balance_cycle = _rt % 4.0
        _wobbling = _balance_cycle < 1.0
        _wobble = math.sin(_balance_cycle * math.pi * 3) * (1.0 - _balance_cycle) * 10 if _wobbling else 0.0
        _lean = math.radians(_wobble)
        _base_y = int(y)

        # Ground shadow
        _shsurf = pygame.Surface((int(hd*3.2), int(hd*0.9)), pygame.SRCALPHA)
        pygame.draw.ellipse(_shsurf, (0,0,0,70), (0,0,int(hd*3.2),int(hd*0.9)))
        surface.blit(_shsurf, (int(x)-int(hd*1.6), _base_y-int(hd*0.45)))

        # Rocky body position (computed first so the leg can attach to it)
        _br = int(hd*1.6)
        ...
        _lumps = [(0.0,-1.0,1.0),(0.55,-0.65,0.9),(0.9,-0.05,0.88),(0.6,0.55,0.9),
                  (0.0,0.85,1.0),(-0.6,0.55,0.9),(-0.9,-0.05,0.88),(-0.55,-0.65,0.9)]
        _pts = [(_cx9+int(math.cos(_lean)*bx*_br-math.sin(_lean)*by*_br*0.2),
                 _cy9+int(math.sin(_lean)*bx*_br*0.3+by*_br*sc)) for bx,by,sc in _lumps]
        pygame.draw.polygon(surface, _rock_col, _pts)
        pygame.draw.polygon(surface, _rock_dk, _pts, max(1,int(2*s)))
        pygame.draw.circle(surface, _rock_lt, (_cx9-int(_br*0.3), _cy9-int(_br*0.35)), int(_br*0.28))
        # Cracks / texture
        for _cxo,_cyo in [(-0.3,0.1),(0.2,0.4),(0.4,-0.2)]:
            pygame.draw.line(surface, _rock_dk, (_cx9+int(_cxo*_br),_cy9+int(_cyo*_br)),
                             (_cx9+int(_cxo*_br)+int(6*s),_cy9+int(_cyo*_br)+int(4*s)), max(1,int(2*s)))

        # Pirate eyepatch + angry brow eye
        _eex = _cx9 + facing*int(_br*0.3); _eey = _cy9 - int(_br*0.15)
        pygame.draw.circle(surface, (255,255,255), (_eex,_eey), max(3,int(hd*0.26)))
        pygame.draw.circle(surface, (20,20,20), (_eex,_eey), max(1,int(hd*0.12)))
        pygame.draw.line(surface, (30,30,30), (_eex-int(8*s),_eey-int(10*s)), (_eex+int(6*s),_eey-int(6*s)), max(2,int(3*s)))
        _pex = _cx9 - facing*int(_br*0.35); _pey = _cy9-int(_br*0.1)
        pygame.draw.circle(surface, (20,20,20), (_pex,_pey), max(4,int(hd*0.3)))
        pygame.draw.line(surface, (60,40,20), (_pex-int(_br*0.4),_pey-int(4*s)), (_pex+int(_br*0.4),_pey+int(6*s)), max(1,int(2*s)))
        # Bandana
        ...
        if action == 'kick':
            return (int(int(x) + facing * int(action_t * 90 * s)), _base_y - int(20*s))
        return (int(_cx9 + facing * 20*s), _cy9)

    # ── Xix!?xy & Zaor@k: two worms sharing a circular handmade monster ─────
    #    costume with gloves — they pop out of holes in the costume.
    if char_name == "Xix!?xy & Zaor@k":
        _xt = pygame.time.get_ticks() / 1000.0
        if action == 'dead':
            return None
        _bob = int(math.sin(_xt*1.8)*3*s)
        _cx9, _cy9 = int(x), int(y) - int(hd*1.7) - _bob
        _br = int(hd*1.85)
        _costume_col = col
        _costume_dk = tuple(max(0,c-35) for c in col)

        # Ground shadow
        _shsurf = pygame.Surface((int(_br*2.2), int(hd*0.8)), pygame.SRCALPHA)
        pygame.draw.ellipse(_shsurf, (0,0,0,70), (0,0,int(_br*2.2),int(hd*0.8)))
        surface.blit(_shsurf, (_cx9-int(_br*1.1), int(y)-int(hd*0.4)))

        # Stubby gloved arms & legs of the handmade costume
        for _lxo in (-int(_br*0.6), int(_br*0.6)):
            pygame.draw.line(surface, _costume_dk, (_cx9+_lxo, _cy9+int(_br*0.6)), (_cx9+_lxo, int(y)), max(4,int(8*s)))
            pygame.draw.circle(surface, (240,240,240), (_cx9+_lxo, int(y)), max(3,int(7*s)))
        for _axo in (-int(_br*1.0), int(_br*1.0)):
            _aang = math.sin(_xt*2+_axo)*0.3
            _ahx = _cx9+_axo+int(math.sin(_aang)*10*s)
            pygame.draw.line(surface, _costume_dk, (_cx9+int(_axo*0.6), _cy9), (_ahx, _cy9+int(_br*0.3)), max(4,int(8*s)))
            pygame.draw.circle(surface, (240,240,240), (_ahx, _cy9+int(_br*0.3)), max(3,int(8*s)))
            pygame.draw.circle(surface, (200,200,200), (_ahx, _cy9+int(_br*0.3)), max(3,int(8*s)), max(1,int(2*s)))

        # Round handmade costume body — stitched fabric look
        pygame.draw.circle(surface, _costume_col, (_cx9, _cy9), _br)
        pygame.draw.circle(surface, _costume_dk, (_cx9, _cy9), _br, max(2,int(3*s)))
        for _sti in range(0, 360, 24):
            _sx0 = _cx9+int(math.cos(math.radians(_sti))*_br*0.97)
            _sy0 = _cy9+int(math.sin(math.radians(_sti))*_br*0.97)
            pygame.draw.circle(surface, _costume_dk, (_sx0,_sy0), max(1,int(2*s)))
        # Painted-on cartoon monster face
        for _fex in (-int(_br*0.3), int(_br*0.3)):
            pygame.draw.circle(surface, (255,255,255), (_cx9+_fex, _cy9-int(_br*0.15)), max(3,int(hd*0.24)))
            pygame.draw.circle(surface, (20,20,20), (_cx9+_fex, _cy9-int(_br*0.15)), max(1,int(hd*0.1)))
        for _tth in range(-2,3):
            pygame.draw.polygon(surface, (250,250,245), [
                (_cx9+_tth*int(6*s), _cy9+int(_br*0.35)), (_cx9+_tth*int(6*s)-int(3*s), _cy9+int(_br*0.5)),
                (_cx9+_tth*int(6*s)+int(3*s), _cy9+int(_br*0.5))])

        # Holes with worms popping out on a staggered cycle
        _hole_specs = [(-0.45,-0.35,0.22),(0.5,-0.25,0.2),(-0.15,0.5,0.2),(0.4,0.45,0.18)]
        for _hi,(hxr,hyr,hr) in enumerate(_hole_specs):
            _hx0 = _cx9+int(hxr*_br); _hy0 = _cy9+int(hyr*_br); _hrad = max(4,int(hr*_br))
            pygame.draw.circle(surface, (15,12,10), (_hx0,_hy0), _hrad)
            _pcycle = (_xt*0.6 + _hi*0.9) % 3.6
            _popped = _pcycle < 1.2
            _pop = math.sin((_pcycle/1.2)*math.pi) if _popped else 0.0
            if _pop > 0.05:
                _wcol = (150,190,70) if _hi % 2 == 0 else (200,110,160)
                _wlen = int(_pop*hr*_br*1.6)
                _wsway = math.sin(_xt*6+_hi)*3*s
                pygame.draw.line(surface, _wcol, (_hx0,_hy0), (_hx0+int(_wsway),_hy0-_wlen), max(3,int(6*s)))
                pygame.draw.circle(surface, _wcol, (_hx0+int(_wsway),_hy0-_wlen), max(2,int(_hrad*0.7)))
                for _wex in (-1,1):
                    pygame.draw.circle(surface, (20,20,20), (_hx0+int(_wsway)+_wex*int(2*s), _hy0-_wlen-int(2*s)), max(1,int(2*s)))

        if action == 'punch':
            return (int(_cx9 + facing * (_br + 22*s)), _cy9)
        if action == 'kick':
            return (int(_cx9 + facing * int(action_t * 90 * s)), int(y) - int(20*s))
        return (int(_cx9 + facing * 20*s), _cy9)
...
# ---------------------------------------------------------------------------
# Jawke, legacy costume — the pencil-sketch version: striped top hat, big
# toothy grin, buttoned belly with a scribbled-out X, a pan in one hand and a
# striped ball in the other. Drawn centred on (cx, cy) so it can be rotated.
# ---------------------------------------------------------------------------

def draw_jawke_legacy(surface, cx, cy, scale=1.0):
    s   = scale
    def P(x, y):
    ...
    pygame.draw.ellipse(surface, _line, (P(-46, -28)[0], P(-46, -28)[1], I(92), I(86)), I(3))

    # ── Legs and big shoes ────────────────────────────────────────────────
    for _lx, _knee in ((-16, True), (18, False)):
        pygame.draw.line(surface, _line, P(_lx, 52), P(_lx - 4, 96), I(4))
        if _knee:                                   # the sketch has one knobbly knee
    ...
    # ── Collar, buttons and the scribbled-out X ───────────────────────────
    pygame.draw.polygon(surface, _skin, [P(-14, -34), P(14, -34), P(8, -22), P(-8, -22)])
    pygame.draw.polygon(surface, _line, [P(-14, -34), P(14, -34), P(8, -22), P(-8, -22)], I(2))
    for _by in (-14, -2, 10):
        pygame.draw.circle(surface, _line, P(0, _by), I(4), I(2))
    for _xs in (-1, 1):                             # scribbled-out X on the chest
        for _xo in (-2, 0, 2):
            pygame.draw.line(surface, _line, P(20 - 14 * _xs + _xo, -20), P(20 + 14 * _xs + _xo, 6), I(2))

    # ── Head: big eyes, wide grin, two front teeth ────────────────────────
    pygame.draw.circle(surface, _skin, P(0, -62), I(35))
    pygame.draw.circle(surface, _line, P(0, -62), I(35), I(3))
    for _ex in (-14, 14):
        pygame.draw.ellipse(surface, (255, 255, 255), (P(_ex - 10, -80)[0], P(_ex - 10, -80)[1], I(20), I(22)))
    ...
    for _tx in (-13, 2):                            # the two big front teeth
        ...
        pygame.draw.rect(surface, _line, (P(_tx, -54)[0], P(_tx, -54)[1], I(12), I(15)), I(2))
